...
export function setMatchesFromTBA(tbaEvent: TBAEvent,tbaMatches:TBAMatch[]) {
  ...
  const matches: SplitterMatch[] = tbaMatches.map(t => (
    {
      id: t.key, name: `${t.comp_level.toUpperCase()}${t.match_number}`, 
...
export function addMatch(match: SplitterMatch, index: number){
  ...
  const newMatches = oldMatches.slice(undefined, index).concat(match).concat(oldMatches.slice(index, undefined))
  useMatches.setState({
    matches: newMatches
  })
}
...
export function addBlankMatch(index: number){
  addMatch({id: uuid(), name: "", description: "", sourceVideoPath: ''}, index)
}
